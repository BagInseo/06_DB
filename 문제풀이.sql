--#varchar2란 : 숫자 영어 한글 등 모든 문자를 넣을 수 있는 공간
--PRIMAYR KEY ㅏ만드러진정보에 기본으로 
...
SELECT MEMBER_NAME 
FROM TB_MEMBER
WHERE GRADE =20 AND MEMBER_NAME LIKE '%이%';

--등급이 '일반회원'인 회원의 이름을 알파벳 순으로 정렬해서
SELECT MEMBER_NAME
FROM TB_MEMBER
WHERE GRADE =10
ORDER BY MEMBER_NAME;

--등급이 '특별회원'이고 이름에 '신'이 포함된
--회원의 아이디와 이름 조회하기
SELECT MEMBERID , MEMBER_NAME 
FROM TB_MEMBER 
...
--특정 지역의 회원 수 조회
SELECT AREA_NAME, COUNT(MEMBERID)  
FROM TB_MEMBER m
JOIN TB_AREA a ON m.AREA_CODE =a.AREA_CODE 
GROUP BY AREA_NAME;

/**/
--특정 회원(hong01)의 지역 정보 조회 
...
--TB_MEMBER 체이블에서 GRADE가 일반회원 이면서 
...
--TB_MEMBER 테이블에서 AREA_CODE가 '031'이거나 '032'인 회원의 이름 조회하기
SELECT MEMBER_NAME
FROM TB_MEMBER 
...
SELECT MEMBERID,GRADE, GRADE_NAME 
FROM TB_MEMBER m 
JOIN TB_GRADE g ON (m.GRADE=g.GRADE_CODE)
...
AND AREA_CODE ='031' --경기도는 031로 숫자 적어줬음
...
FROM TB_MEMBER tm 
JOIN TB_GRADE tg ON (tm.GRADE=tg.GRADE_CODE)
WHERE MEMBER_NAME LIKE '%유%';

--17: TB_MEMBER 테이블에서 등급이 '일반회원'이면서 가입일이 현재 날짜보다 이전인 회원의 수를 조회
SELECT COUNT(*)
FROM TB_MEMBER 
WHERE GRADE ='10' -- 일반회원은 10으로 숫자 적어줬음
AND "JOIN_DATE(내가 나중에 사용자가 가입한 날짜를 표시해줄
 이름을 작성)">SYSDATE ;
--JOIN_DATE : 사용자가 가입한 날짜를 나타냄
--SYSDATE : 내 컴퓨터에서 보이는 현재날짜 현재시간
...
JOIN TB_GRADE tg ON (tm.GRADE=tg.GRADE_CODE)
JOIN TB_AREA ta ON (tm.AREA_CODE=ta.AREA_CODE)
WHERE GRADE_NAME='일반회원' AND AREA_NAME ='서울'
ORDER BY GRADE DESC;
...
SELECT MEMBERID , MEMBER_NAME 
FROM TB_MEMBER
WHERE CHAR_LENGTH(MEMBER_NAME)>=4 ; 

--22: TB_MEMBER 테이블에서 등급이 '특별회원'이면서 이름이 '김영희'가 아닌 회원의 아이디를 조회
SELECT MEMBERID
FROM TB_MEMBER tm 
JOIN TB_GRADE tg ON (tm.GRADE=tg.GRADE_CODE)
WHERE GRADE_NAME ='특별회원' AND MEMBER_NAME != '김영희';

--23: TB_MEMBER 테이블에서 등급이 '일반회원'인 회원 중에서 이름이 '김영희'이거나 '아이유'인 회원의 수를 조회
SELECT COUNT(*) 
FROM TB_MEMBER tm 
JOIN TB_GRADE tg ON (tm.GRADE=tg.GRADE_CODE)
WHERE GRADE_NAME ='일반회원' AND MEMBER_NAME IN ('김영희','아이유');
...
FROM TB_MEMBER tm 
JOIN TB_GRADE tg ON (tm.GRADE=tg.GRADE_CODE)
WHERE GRADE_NAME !='특별회원' AND MEMBER_NAME!= '홍길동';
...
JOIN TB_GRADE tg ON (tm.GRADE=tg.GRADE_CODE)
JOIN TB_AREA ta ON (tm.AREA_CODE=ta.AREA_CODE)
WHERE GRADE_NAME ='일반회원' AND AREA_NAME IN ('인천','경기');
...
FROM TB_MEMBER tm 
JOIN TB_GRADE tg ON (tm.GRADE=tg.GRADE_CODE)
WHERE GRADE_NAME ='일반회원' AND MEMBER_NAME LIKE '%김%';
