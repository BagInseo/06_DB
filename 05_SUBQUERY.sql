...
-- WHERE EXSITS (서브쿼리)
 --> 서브쿼리 결과(RESULT SET)에 행이 존재 한다며 TRUE
 --> 없으면  FALSE 
...
-- 6. 스칼라 서브쿼리 (SELSCT절에 작성하는 단일행 서브쿼리)
...
ORDER BY SALARY DESC;
-- [문제 발생]--
 -->SELSCT절이 ORDER BY 보다 먼저 해석되어
 -->ROWNUM 순서가 급여 순서가 아닌, 조회된 행 순서조 지정됨
...
FROM (
	...
	SELECT EMP_NAME,SALARY
	FROM EMPLOYEE 
	ORDER BY SALARY DESC  --> 조회 결과가 전채하면
		)
	WHERE ROWNUM <=5;
 ...
 *    EX) 상관 쿼리 -> 테이블별 별칭 지정
 * 
 *    EX) 다른 테이블이여도 컬럼명이 같을 때 -> 테이블별 별칭 지정
 * 																 -> 데이블명.컬럼명	
 * 
