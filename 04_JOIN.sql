...
/*JOIN은 단순히 테이클 두개를 붙이는 것이 아닌
 * 
 * 기준 세운 테이블의 한 컬럼을 지정해
...
-->모든 경우의 수를 보고싶을 때 빼고는 직접 작성하는 겨우가 없음
 -->자연 조인 실패 결과로 교차 조닝 결과가 풀력ㄷㅚㅁ
  -->(교차, 조인 결과가 보이면 )->>아.. 자연 조인 잚봇ㅆ넹)

SELECT EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE
CROSS JOIN DEPARTMENT
ORDER BY EMP_NAME, DEPT_CODE; --오류 발생
...
--     LOCATION_ID 컬럼이 존재하지 않아서 JOIN 실패
...
-- SELF JOIN 사용
SELECT EMP.EMP_NAME ,EMP.DEPT_CODE ,MEM.EMP_NAME
FROM EMPLOYEE EMP
JOIN EMPLOYEE MEM ON (EMP.DEPT_CODE=MEM.DEPT_CODE)
WHERE EMP.EMP_NAME != MEM.EMP_NAME
ORDER BY 1;
