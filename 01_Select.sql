...
/*2*/SELECT EMP_NAME , SALARY 
/*1*/FROM  EMPLOYEE 
/*3*/ORDER BY SALARY DESC ; --DESC(descending) : 오름차순
...
/* ORDER BY절에 수식 적용 */
--EMPLOYEE 테이블에서 이름, 연봉을 내림파순으로 조회
SELECT EMP_NAME, SALARY *12 
FROM EMPLOYEE 
ORDER BY SELECT  *12 DESC;

--정렬 시 SELECT절에 작성되 너컬럼을
--그대로 따라 적는 경우가 많ㅇㄷ



/* ORDER BY절에 별칭 사용 하기 */
--->SELECT절 해석 이후 order by가 해석되기 때문에
...
SELECT EMP_NAME, PHONE FROM EMPLOYEE
ORDER BY PHONE DESC NULL FIRST; --정렬 기준 -> NULL위치 해석 순서대로 해석
...
FROM EMPLOYEE 
ORDER BY DEPT_CODE, SALARY  SALARY DESC ;



-- EMPLOYEE 테이블에서
-- 이름, 부서코드, 직접코드(JIB_CODE)를
-- 부서코드 오름차순,  직접코드 내림 차순,이름 오름차순으로 조회
