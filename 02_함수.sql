...
-- 이메일 아이디 오름차순으로 조회
SELECT EMP_NAME,1, SUBSTR(EMAIL,INSTR(EMAIL,'@'ㄴㄴ), -1) "이메일 아이디" 
FROM EMPLOYEE
ORDER BY "이메일 아이디";
...
-- 문자열 공백 제거
SELECT '   기  준   ',
				TRIM(LEADING, ' 'FROM'   기  준   '), --앞꽂 공백 작성 
				TRIM(LEADING, ' 'FROM'   기  준   '), --앞꽂 공백 작성 
				TRIM(BOTH, ' 'FROM'   기  준   ') --앞꽂 공백 작성 
				FROM DUAL ;
...
--MONTHS_BETWEEN()은 윤년(2/29일이 포함된 해) 계산이 자동으로 수행
...
SELECT SYSDATE , TO_CHAR(SYSDATE,'YY/MM/DD (DAY) PM HH:MI:SS') 
FROM DUAL;
...
-- 현재 날짜를 ->2024년 02월 27일 오후 12시 15분 30초
...
-- <NULL 처리 연산> : IS NULL/ IN NOT NULL
...
/*NULL 이랑 산술 연산 수행 시 결과는 무조건 NULL !!!*/
...
DECODE(JOB_CODE ,
'J7',NVL(SALARY*1.1,0) ,
'J6',NVL(SALARY*1.15,0) ,
'J5',NVL(SALARY*1.2,0),
NVL(SALARY*1.05,0))"지급급여"
...
FROM EMPLOYEE 					/*1)EMPLOYEE 테이블 23행 중		*/
WHERE DEPT_CODE ='D6';  /*2)DEPT_CODE가 'D6'인 행만 모아서*/
